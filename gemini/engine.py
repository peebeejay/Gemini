...
# Local imorts
from gemini import exchange, helpers
...
class backtest():
    ...
    def __init__(self, data):
        ...
        missing = set(['date', 'high', 'low', 'open', 'close', 'volume']
                      ) - set(data.columns)
        if len(missing) > 0:
    ...
    def start(self, initial_capital, logic):
        ...
        df['benchmark_return'] = (
            df.benchmark_equity-df.benchmark_equity.shift(1)) / df.benchmark_equity.shift(1)
        df['strategy_return'] = (
            df.strategy_equity-df.strategy_equity.shift(1)) / df.strategy_equity.shift(1)
        df.index = df['date']
        del df['date']
        return df
    ...
    def results(self):
        ...
        longs = len(
            [t for t in self.account.opened_trades if t.type == 'long'])
        sells = len(
            [t for t in self.account.closed_trades if t.type == 'long'])
        shorts = len(
    ...
    def chart(self, show_trades=False, title="Equity Curve"):
        ...
        p.line(self.data['date'], base_equity,
               color='#CAD8DE', legend='Buy and Hold')
        p.line(self.data['date'], self.account.equity,
               color='#49516F', legend='Strategy')
